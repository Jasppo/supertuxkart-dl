...
class Match:
    ...
    # JASPO
    def _to_image_400_300(self, x, proj, view):
        W, H = 400/2, 300/2
        p = proj @ view @ np.array(list(x) + [1])
        coords_1 = np.clip([W * (1 + (p[0]/p[-1]))], 0, 400)
        coords_2 = np.clip([H * (1 + (-p[1]/p[-1]))], 0, 300)
        screen_coords = np.array([coords_1[0], coords_2[0]])
        if p[2] < 0 and p[3] < 0:
            return np.array([200., 300.])
        else:
    ...
    def _to_image(self, x, proj, view):
        ...
        if p[2] < 0 and p[3] < 0:
            ...
            return np.clip(np.array([p[0] / p[-1], -p[1] / p[-1]], dtype=np.float32), -1., 1.)

    def _to_image1(self, x, proj, view):
        p = proj @ view @ np.array(list(x) + [1])
        
        s_coords = np.array([p[0] / p[-1], -p[1] / p[-1]], dtype = np.float32)
        
        if abs(s_coords[0]) > 1 and abs(s_coords[0]) < 1.2 and abs(s_coords[1]) < 0.1:
            s_coords = np.clip(s_coords, -1, 1)
            
        if (p[2] < 0 and p[3] < 0) or abs(s_coords[0]) > 1 or abs(s_coords[1]) > 1:
            return np.array([0., 1.])
        else:
            return np.clip(s_coords, -1, 1)

    # JASPO 
    def _save_data(self, image, mask, match_number, player_number, iteration, proj, view, ball_location, puck_only="no"):
        import PIL.Image
        import os
        import torch
        path_to_save = 'data/'
        mask = (mask >> 24 == 8).astype(int)
        # Number of pixels
        tmask = torch.from_numpy(mask)
        n_puck_pixels = torch.sum(tmask).item()
        if n_puck_pixels >= 20:
            label = '1'
        else:
            label = '0'
        sc = self._to_image(ball_location, proj, view)
        if puck_only == "no" or (sc[0] != 0. and sc[1] != 1.):
            PIL.Image.fromarray(image).save(os.path.join(path_to_save, 'M%02d_player%02d_%04d_im.png' % (match_number, player_number, iteration)))
            PIL.Image.fromarray((mask * 255).astype(np.uint8)).save(os.path.join(path_to_save, 'M%02d_player%02d_%04d_seg.png' % (match_number, player_number, iteration)))
            puck_loc = os.path.join(path_to_save, 'M%02d_player%02d_%04d_aim.csv' % (match_number, player_number, iteration))
            with open(puck_loc, 'w') as f:
                f.write('%.4f, %.4f' % tuple(self._to_image(ball_location, proj, view)))
            puck_classify = os.path.join(path_to_save, 'M%02d_player%02d_%04d_classify.csv' % (match_number, player_number, iteration))
            with open(puck_classify, 'w') as f:
                f.write(tuple(label))

    def run(self, team1, team2, num_player=1, max_frames=MAX_FRAMES, max_score=3, record_fn=None, timeout=1e10,
            initial_ball_location=[0, 0], initial_ball_velocity=[0, 0], verbose=False, save_data='no', puck_only = 'no', match_number=1):
        ...
        for it in range(max_frames):
            ...
            if self._use_graphics:
                ...
                team1_puck_mask = [np.array(race.render_data[i].instance) for i in range(0, len(race.render_data), 2)]
                team2_puck_mask = [np.array(race.render_data[i].instance) for i in range(1, len(race.render_data), 2)]
            ...
            if save_data == 'yes':
                for i in range(2):
                    self._save_data(image = team1_images[i], mask = team1_puck_mask[i], match_number = match_number, player_number = i, iteration = it, 
                        proj = team1_state[i]['camera']['projection'].T, view = team1_state[i]['camera']['view'].T, ball_location = soccer_state['ball']['location'], 
                        puck_only = puck_only)

                    self._save_data(image = team2_images[i], mask = team2_puck_mask[i], match_number = match_number, player_number = i+2, iteration = it, 
                        proj = team2_state[i]['camera']['projection'].T, view = team2_state[i]['camera']['view'].T, ball_location = soccer_state['ball']['location'], 
                        puck_only = puck_only)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-sd', '--save_data', help="Do you want to generate training data?", choices = ['yes', 'no'], default = 'no')
    parser.add_argument('-po', '--puck_only', help="Do you want to only generate images with puck on screen?", choices = ['yes', 'no'], default = 'no')
    parser.add_argument('-mn', '--match_number', help="Match number - Used for generating training data", default = 1, type=int)
    ...
    if args.parallel is None or remote.ray is None:
        ...
        try:
            result = match.run(team1, team2, args.num_players, args.num_frames, max_score=args.max_score,
                               ...
                               record_fn=recorder, save_data = args.save_data, puck_only = args.puck_only, match_number=args.match_number)
